...
export const appSettings = (): Promise<{ defect_required_fields: string[]; check_view_days?: number; signoff_view_days?: number; auto_logout_minutes?: number; leon_offline_flights?: number; amm_revision?: string; field_config?: Record<string, Record<string, { visible?: boolean; required?: boolean; label?: string }>>; mandatory_fields?: Record<string, Record<string, boolean>> }> =>
  api('/admin/settings');

export const deleteDefect = (id: string, approvedBy: string, force = false) =>
  mutateOrQueue(`/defects/${id}?approved_by=${encodeURIComponent(approvedBy)}${force ? '&force=true' : ''}`, { method: 'DELETE' });
