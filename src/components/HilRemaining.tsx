...
// A column reads "—" when that limit was not set; whichever remaining reaches zero (or below)
// shows red. Remaining is measured from the date the HIL was registered: Days count to the due
// date, Hrs/Cyc from the TSN/CSN stamped at deferral.
        ...
        <View style={{ flexDirection: 'row' }}>
          {cols.map((c) => (
            <View key={c.label} style={{ alignItems: 'center', minWidth: 50 }}>
              ...
              <Text style={{ fontSize: 15, fontWeight: '800', fontVariant: ['tabular-nums'], color: !c.set ? theme.sub : (c.red ? theme.red : theme.text) }}>{c.set ? c.txt : '—'}</Text>
            </View>
          ))}
        </View>
      )}
